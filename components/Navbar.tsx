...
const Navbar: React.FC = () => {
    ...
    return (
        <nav className="sticky top-0 z-50 w-full border-b border-[#2c4823] bg-background-dark/95 backdrop-blur-md">
            <div className="px-4 md:px-10 py-4 flex items-center justify-between max-w-7xl mx-auto relative z-50">
                <Link to="/" className="flex items-center gap-3 text-white group cursor-pointer" onClick={() => setIsMenuOpen(false)}>
                    <img 
                        src={logoUrl} 
                    ...
                    <div className="flex flex-col">
                        <h2 className="text-white text-lg md:text-xl font-bold leading-none tracking-tight">Summit Turf</h2>
                ...
                <div className="hidden lg:flex items-center gap-6 xl:gap-8">
                    {navLinks.map((link) => (
                        <Link 
                            key={link.path}
                            className={`text-sm font-bold transition-colors duration-300 ${location.pathname === link.path ? 'text-primary' : 'text-gray-300 hover:text-white'}`} 
                            to={link.path}
                        >
                            {link.name}
                        </Link>
                    ))}
                    <Link className={`text-sm font-bold transition-colors duration-300 ${location.pathname === '/contact' ? 'text-primary' : 'text-gray-300 hover:text-white'}`} to="/contact">Contact</Link>
                </div>
                ...
                <div className="hidden lg:flex items-center gap-6">
                    <button 
                        ...
                        className="cursor-pointer flex items-center justify-center rounded-full h-11 px-7 bg-primary text-background-dark text-sm font-bold hover:bg-white hover:scale-105 transition-all duration-300 shadow-[0_0_20px_rgba(70,236,19,0.2)]"
                    >
                        Get a Quote
                    </button>
                </div>
                ...
                <button 
                    className="lg:hidden relative z-50 w-14 h-14 -mr-2 flex items-center justify-center text-white hover:text-primary transition-colors rounded-full focus:outline-none focus:bg-white/5 active:scale-95 touch-manipulation"
            ...
            {/* Mobile Menu Overlay */}
            <div 
                className={`fixed inset-0 w-full h-screen bg-background-dark/98 backdrop-blur-xl z-40 transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) lg:hidden flex flex-col ${
                    isMenuOpen 
                ...
                <div className="flex-1 flex flex-col justify-start px-6 pt-32 pb-10 overflow-y-auto min-h-full">
                    <div className="flex flex-col gap-6 items-center text-center">
                        {navLinks.map((link, index) => (
                            <Link 
                                key={link.path}
                                className={`text-2xl font-bold transition-all duration-500 ${location.pathname === link.path ? 'text-primary' : 'text-white hover:text-primary'}`}
                                to={link.path}
                        ...
                        <Link 
                            to="/contact"
                            className={`text-2xl font-bold transition-all duration-500 ${location.pathname === '/contact' ? 'text-primary' : 'text-white hover:text-primary'}`}
                            onClick={() => setIsMenuOpen(false)}
                            style={{ 
                        ...
                        <button 
                            ...
                            className="mt-8 w-full max-w-xs h-14 rounded-full bg-primary text-background-dark text-lg font-bold hover:bg-white transition-all shadow-xl hover:scale-105 active:scale-95"
                            style={{ 
                    ...
                    <div className="mt-auto flex flex-col items-center gap-6 pt-12 pb-8">
                        ...
                        <div className={`flex flex-col items-center gap-3 transition-opacity duration-700 delay-500 ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`}>
                            <a href="tel:5551234567" className="flex items-center gap-2 text-gray-300 hover:text-primary transition-colors">
